...
export const CONFIG = {
  ...
  stick: {
    ...
    // アクションは「指を払った」ときだけ出る。ただ離しただけでは何も起きない。
    // 進行方向の操舵は常に「置いた地点 → 今の指」だが、アクションの向きは
    // 指を加速させた瞬間の地点から離した地点までを見る。
    // ゆっくり動かして向きを整えてから、パッと払った方向へ蹴る。
    flickSpeed: 0.28,   // CSS px/ms（= 280px/s）。これを超えた区間をフリックとみなす
    flickDist: 12,      // CSS px。その区間の移動量がこれ未満なら無視する
    flickMaxMs: 220,    // これより古くは遡らない（遅い払いは時間が長くなるので広めに取る）
    flickRestMs: 90,    // 払ったあとこれ以上その場で止まっていたら、払っていない扱い
  },
