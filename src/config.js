...
export const CONFIG = {
  ...
  stick: {
    ...
    // スワイプする必要はなく、倒したまま静止して離してもよい。
    // 中央へ戻してから離せば何も起きない。
    releaseTilt: 0.2,

    // 「最後に指を走らせた向き」を拾うための値。
    // 進行方向の操舵は常に「置いた地点 → 今の指」だが、アクションの向きだけは
    // 指を加速させた瞬間の地点から離した地点までを見る。
    // ゆっくり動かして向きを整えてから、パッと払った方向へ蹴れるようにするため。
    flickSpeed: 0.5,    // CSS px/ms（= 500px/s）。これを超えた区間をフリックとみなす
    flickDist: 14,      // CSS px。その区間の移動量がこれ未満なら無視する
    flickMaxMs: 160,    // これより古くは遡らない
  },
